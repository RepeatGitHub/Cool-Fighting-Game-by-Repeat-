        ...
        for (var a=0;a<p.length;a++) {
            ...
            if (myImages[p[a].char][p[a].frame1][p[a].frame2]===undefined) {
                ...
                if (p[a].dir===1) {
                    ...
                    if (p[a].hp>=100) {
                        repeatsImageGenerator(myImages[p[a].char][p[a].frame1][p[a].frame2],p[a].x,p[a].y,48,myImages[p[a].char][p[a].frame1][p[a].frame2][0].length*3,color(255,0,0,min(p[a].hp-100,200)*(sin(frameCount*5)/2+0.5)));
                    }
                } else {
                    if (!showHitboxes) {
